    ...
    try {
      ...
      if (cover) {
        try {
          ...
          doc.image(cover, M, yL, { fit: [LOGO_W, LOGO_H] });
          doc.restore();
          yL += LOGO_H + 10;
        } catch { /* skip on error */ }
      }
      ...
      const renderItems = (sy: number): number => {
        ...
        const smallRows: [string, string][] = [
          ...
          ...(discountAmount > 0
            ? ([[`DESCUENTO${input.discountPercent ? ` (${input.discountPercent}%)` : ""}`, `-${formatCurrency(discountAmount, input.currency)}`]] as [string, string][])
            : []),
          ...(input.taxAmount
