    ...
    try {
      ...
      if (cover) {
        try {
          ...
          doc.image(cover, M, (HDR_H - LH) / 2, { fit: [LW, LH] });
          doc.restore();
          logoRight = M + LW + 14;
        } catch { /* skip */ }
      }
      ...
      doc.fillColor(hdrTxt).font(FONT_BOLD).fontSize(26)
      ...
      const ensureSpace = (cy: number, need: number, withH = false) => {
        ...
        doc.fillColor(hdrTxt).font(FONT_BOLD).fontSize(11)
      ...
      if (lines.length === 0) {
        ...
        doc.fillColor(inkDim).font(FONT_REGULAR).fontSize(9).text("Sin ítems.", dX + cP, y + 10);
        y += 32;
      }
      ...
      const boxData = [
        ...
        ...(input.taxAmount
          ...
          : []),
        { label: "TOTAL",     value: formatCurrency(input.total, input.currency), accent: true  },
      ];
      const BOX_W = Math.floor(CW / boxData.length) - 6;
      const BOX_H = 44;
      boxData.forEach((b, i) => {
        ...
        doc.fillColor(b.accent ? hdrTxt : inkDim).font(FONT_BOLD).fontSize(7.5)
           .text(b.label, bx + 8, y + 8, { width: BOX_W - 14, align: "center" });
        doc.fillColor(clr).font(FONT_BOLD).fontSize(13)
      ...
      resolveCustomFields(input, "footer").forEach((f) => {
        y = ensureSpace(y, 30);
        y = renderCustomField(f, M, y, CW, "center");
      });
