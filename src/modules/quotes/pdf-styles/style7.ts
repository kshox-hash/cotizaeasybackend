...
// Estilo 7 — Ficha ejecutiva: avatar circular + marca a la izquierda, título
// limpio a la derecha con una tarjeta de bordes redondeados para los metadatos.
export function generateStyle7(
  input: QuotePdfInput,
  cover: Buffer | null,
  fileName: string,
  filePath: string,
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const drawContHeader = () => {
        doc.circle(M + 5, 19, 5).fill(accent);
        doc.fillColor(ink).font(FONT_BOLD).fontSize(11)
           .text(brand, M + 16, 13, { width: CW * 0.6 });
        doc.fillColor(inkDim).font(FONT_REGULAR).fontSize(8)
           .text(`Cotización N° ${qNumber}  ·  ${issueDate}`, M, 14, { width: CW, align: "right" });
        hLine(32, border, 0.6);
      };
      ...
      // ── Avatar circular + marca (izquierda) ───────────────────────────────
      const AV_R = 27;
      const avX = M + AV_R;
      const avY = M + AV_R;
      if (cover) {
        try {
          doc.save();
          doc.circle(avX, avY, AV_R).clip();
          doc.image(cover, M, M, { fit: [AV_R * 2, AV_R * 2] });
          doc.restore();
        } catch { /* fallback: sin logo, solo iniciales */ }
      } else {
        doc.circle(avX, avY, AV_R).fill(accent);
        doc.fillColor(hdrTxt).font(FONT_BOLD).fontSize(20)
           .text((brand[0] || "M").toUpperCase(), M, M + AV_R - 12, { width: AV_R * 2, align: "center" });
      }
      doc.strokeColor(border).lineWidth(1).circle(avX, avY, AV_R).stroke();

      const TXT_X = M + AV_R * 2 + 16;
      const LEFT_W  = Math.round(CW * 0.54);
      const TXT_W = M + LEFT_W - TXT_X;
      let tY = M + 4;
      doc.fillColor(ink).font(FONT_BOLD).fontSize(15)
         .text(brand, TXT_X, tY, { width: TXT_W });
      tY += strH(brand, FONT_BOLD, 15, TXT_W) + 4;
      const infoBits = [input.brandAddress, input.brandRut ? `RUT: ${input.brandRut}` : undefined, input.brandPhone ? `Tel: ${input.brandPhone}` : undefined].filter(Boolean);
      infoBits.forEach((line) => {
        doc.fillColor(inkSub).font(FONT_REGULAR).fontSize(8.5).text(line as string, TXT_X, tY, { width: TXT_W });
        tY += 13;
      });

      // ── Título + tarjeta redondeada de metadatos (derecha) ────────────────
      const RIGHT_X = M + LEFT_W + 14;
      const RIGHT_W = CW - LEFT_W - 14;
      doc.fillColor(ink).font(FONT_BOLD).fontSize(24)
         .text("Cotización", RIGHT_X, M, { width: RIGHT_W, align: "right" });

      const CARD_Y = M + 34;
      const CARD_H = 62;
      doc.roundedRect(RIGHT_X, CARD_Y, RIGHT_W, CARD_H, 10).fillAndStroke(rowAlt, border);
      ...
      cardRows.forEach(([label, value]) => {
        doc.circle(RIGHT_X + 12, cry + 4, 2.5).fill(accent);
        doc.fillColor(inkDim).font(FONT_BOLD).fontSize(7.5)
           .text(label, RIGHT_X + 20, cry, { width: 90 });
        doc.fillColor(ink).font(FONT_BOLD).fontSize(8.5)
           .text(value, RIGHT_X + 20, cry, { width: RIGHT_W - 30, align: "right" });
        cry += 16;
      });

      let y = Math.max(tY, CARD_Y + CARD_H) + 20;
      hLine(y, border, 0.8);
      y += 13;
